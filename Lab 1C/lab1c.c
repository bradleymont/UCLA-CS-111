...
//this function is from the following website: https://www.gnu.org/software/libc/manual/html_node/Elapsed-Time.html
//the TAs have allowed us to use this function
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        if (profile)
        {
            //measure the user and system time before execution
            getrusage(RUSAGE_SELF, &preExecutionUsage);
            
            preExecUserTime = preExecutionUsage.ru_utime;
            preExecSystemTime = preExecutionUsage.ru_stime;
        }
        ...
        if (profile && !profilePassedThisIteration)
        {
            ...
            struct timeval postExecUserTime = postExecutionUsage.ru_utime;
            struct timeval postExecSystemTime = postExecutionUsage.ru_stime;
            
            //find the difference in user time
            struct timeval userDiff;
            timeval_subtract(&userDiff, &postExecUserTime, &preExecUserTime);
            
            double userTime = userDiff.tv_sec + (userDiff.tv_usec / 1000000.0);
            printf("user: %fs, ", userTime);
            fflush(stdout);
            
            //find the difference in system time
            struct timeval systemDiff;
